...
int find_display_gpu(int gpu_number, uint32_t gpu_count, VkPhysicalDevice *physical_devices) {
    ...
    if (gpu_number >= 0) {
        ...
        for (uint32_t i = 0; i < gpu_count; i++) {
            ...
            printf("GPU count = %d, vkGetPhysicalDeviceDisplayPropertiesKHR result = %d\n", gpu_count, result);
            assert(!result);
            if (display_count) {
                gpu_return = i;
                break;
...
static void demo_init_vk(struct demo *demo) {
    ...
    VkInstanceCreateInfo inst_info = {
        ...
        .pNext = NULL,
        .flags = 0,//(portabilityEnumerationActive ? VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR : 0),
